...
function ratingValues(
  question: Extract<Question, { type: "rating" }>,
): Array<{ value: bigint; label: string }> {
  ...
  switch (question.type) {
    case "singleChoice":
      return draft.type === "singleChoice"
        ...
        : false;
    case "multiSelect":
      return draft.type === "multiSelect" &&
        draft.optionIndices.length >= question.minSelections &&
        draft.optionIndices.length <= question.maxSelections
        ? {
            type: "multiSelect",
            questionIndex: index,
    ...
    case "ranking":
      return draft.type === "ranking" &&
        draft.ranking.length >= question.minRanked &&
        draft.ranking.length <= question.maxRanked
        ? { type: "ranking", questionIndex: index, ranking: draft.ranking }
        : false;
    case "numericRange": {
      if (draft.type !== "numeric" || !/^-?\d+$/.test(draft.value))
        return false;
      const value = BigInt(draft.value);
      const step = question.constraints.step;
      return value >= question.constraints.min &&
        value <= question.constraints.max &&
        (!step || (value - question.constraints.min) % step === BigInt(0))
        ? { type: "numeric", questionIndex: index, value }
        : false;
    }
    case "pointsAllocation": {
      if (
        draft.type !== "pointsAllocation" ||
        draft.points.some((points) => !/^\d+$/.test(points))
      )
        return false;
      const exactPoints = draft.points.map(BigInt);
      if (exactPoints.some((value) => value > BigInt(question.budget)))
        return false;
      const points = exactPoints.map(Number);
      return exactPoints.reduce((sum, value) => sum + value, BigInt(0)) ===
        BigInt(question.budget)
        ? {
            type: "pointsAllocation",
            questionIndex: index,
            allocations: points.flatMap((value, optionIndex) =>
              value > 0 ? [{ optionIndex, points: value }] : [],
            ),
          }
        : false;
    }
    case "rating": {
      ...
      if (
        draft.ratings.some((value) => value !== null && !/^-?\d+$/.test(value))
      )
        return false;
      ...
      const validRating = ({ rating }: { rating: bigint }) => {
        if (question.scale.type === "labels")
          return (
            rating >= BigInt(0) && rating < BigInt(question.scale.labels.length)
          );
        if (question.scale.type === "count")
          return rating >= BigInt(0) && rating < BigInt(question.scale.count);
        const { min, max, step } = question.scale.constraints;
        return (
          rating >= min &&
          rating <= max &&
          (!step || (rating - min) % step === BigInt(0))
        );
      };
      return ratings.length > 0 &&
        (!question.requireAll || ratings.length === draft.ratings.length) &&
        ratings.every(validRating)
        ? { type: "rating", questionIndex: index, ratings }
        : false;
    }
    case "custom":
      // Method-specific semantics are not implemented by this release.
      return false;
  }
}

export function Cip179ResponseForm({
  definition,
  disabled = false,
  onAnswersChange,
}: Props) {
  ...
  useEffect(() => onAnswersChange(answers), [answers, drafts, onAnswersChange]);
    ...
    <div className="space-y-3">
          ...
          <div key={index} className="border border-border p-3">
            ...
            {question.type === "singleChoice" ? (
              <div className="mt-2 flex flex-wrap gap-2">
                {labels.map((label, optionIndex) => (
                  <Button
                    ...
                    disabled={disabled}
                    onClick={() =>
                      update(index, { type: "singleChoice", optionIndex })
                    }
                  >
                    {label}
                  </Button>
              ...
              <div className="mt-2 space-y-1">
                {labels.map((label, optionIndex) => {
                  const selected =
                    draft?.type === "multiSelect" ? draft.optionIndices : [];
                  return (
                    <label key={optionIndex} className="flex gap-2 text-sm">
                      <input
                        type="checkbox"
                        ...
                        onChange={(event) =>
                          update(index, {
                            type: "multiSelect",
                            optionIndices: event.target.checked
              ...
              <div className="mt-2 space-y-2">
                {labels.map((label, optionIndex) => {
                  const ranking =
                    draft?.type === "ranking" ? draft.ranking : [];
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        disabled={
                          disabled ||
                          (position < 0 && ranking.length >= question.maxRanked)
                        }
                        onChange={(event) =>
                          update(index, {
                            type: "ranking",
                            ranking: event.target.checked
                              ? [...ranking, optionIndex]
                              : ranking.filter(
                                  (value) => value !== optionIndex,
                                ),
                        ...
                        <>
                          <Button
                            ...
                            onClick={() => {
                              ...
                              update(index, { type: "ranking", ranking: next });
                            }}
                          >
              ...
              <input
                ...
                type="number"
                min={String(question.constraints.min)}
                max={String(question.constraints.max)}
                step={String(question.constraints.step ?? BigInt(1))}
                disabled={disabled}
                value={draft?.type === "numeric" ? draft.value : ""}
                onChange={(event) =>
                  update(index, { type: "numeric", value: event.target.value })
                }
              />
              ...
              <div className="mt-2 space-y-2">
                {labels.map((label, optionIndex) => {
                  ...
                  return (
                    ...
                    >
                      <span>{label}</span>
                      <input
                        className="h-8 w-24 border border-input bg-background px-2"
                        type="number"
                        min={0}
                        max={question.budget}
                        step={1}
                        disabled={disabled}
                        value={points[optionIndex]}
                        onChange={(event) => {
                          const next = [...points];
                ...
                <p
                  className={cn(
                    "text-xs",
                    draft?.type === "pointsAllocation" &&
                      answerFor(question, index, draft) !== false
                      ? "text-muted-foreground"
                      : "text-amber-700",
                  )}
                >
                  Allocate exactly {question.budget} points.
                </p>
              </div>
                    ...
                    >
                      ...
                      {question.scale.type === "numeric" ? (
                        <input
                          className="h-8 w-24 border border-input bg-background px-2"
                          type="number"
                          min={String(question.scale.constraints.min)}
                          max={String(question.scale.constraints.max)}
                          step={String(
                            question.scale.constraints.step ?? BigInt(1),
                          )}
                          disabled={disabled}
                          ...
                          onChange={(event) => {
                            ...
                            update(index, { type: "rating", ratings: next });
                          }}
                        />
                      ) : (
                        <select
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                Custom response methods are not supported by this CGov release.
              </p>
            )}

            {draft && !question.required ? (
              <Button
                type="button"
                variant="ghost"
                size="sm"
      ...
      {answers === null ? (
        <p className="text-xs text-amber-700">
          Complete all required questions and correct invalid values to attach
