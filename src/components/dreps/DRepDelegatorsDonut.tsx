...
interface DRepDelegatorsDonutProps {
  dreps: DRepSummary[];
  className?: string;
}
...
export function DRepDelegatorsDonut({ dreps, className }: DRepDelegatorsDonutProps) {
  ...
  const sorted = useMemo(() => {
    return [...dreps]
      .filter((d) => (d.delegatorCount ?? 0) > 0)
      .sort((a, b) => (b.delegatorCount ?? 0) - (a.delegatorCount ?? 0));
  }, [dreps]);

  const data = useMemo<SliceData[]>(() => {
    const top = sorted.slice(0, topN);
    const rest = sorted.slice(topN);
    const othersValue = rest.reduce((sum, d) => sum + (d.delegatorCount ?? 0), 0);
    ...
    const slices: SliceData[] = top.map((d, i) => {
      ...
      return { name, value: d.delegatorCount ?? 0, fill };
    });
  ...
  const total = useMemo(() => sorted.reduce((s, d) => s + (d.delegatorCount ?? 0), 0), [sorted]);

  // Concentration stats for legend
  const concentration = useMemo(() => {
    const cumSum = (n: number) => sorted.slice(0, n).reduce((s, d) => s + (d.delegatorCount ?? 0), 0);
  ...
  return (
    <div className={className}>
      ...
      <h4 className={`text-xs font-semibold uppercase tracking-wide mb-2 text-center ${mutedColor}`}>
        {t("drep.delegatorConcentrationTitle")}
      </h4>
      ...
      <div className="flex items-center">
        ...
        <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
          <PieChart style={{ overflow: "visible" }}>
            {isLight && (
              <defs>
                <filter id="delegatorsDonutShadow" x="-50%" y="-50%" width="200%" height="200%">
            ...
            <Pie
              ...
              stroke="none"
              style={isLight ? { filter: "url(#delegatorsDonutShadow)" } : undefined}
            >
          ...
          {[
            ...
            return (
              <div key={label} className={`flex items-center gap-3 text-xs rounded px-1 -mx-1 transition-colors ${
                ...
                <span className={`w-[45px] text-right font-medium ${textColor}`}>{value.toLocaleString()}</span>
