...
async function fetchDrepVotingPower(
  drepId: string,
  epochNo: number
): Promise<{ value?: string; rawRow?: KoiosDrepVotingPowerRow; error?: KoiosErrorDetail }> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return {
        error: {
          status: res.status,
          message: `Koios drep_voting_power_history returned HTTP ${res.status}`,
          bodySnippet,
        },
      };
    }
    ...
    if (!Array.isArray(data)) {
      return {
        error: {
          message: "Koios drep_voting_power_history returned non-array response",
        },
      };
    }
    if (data.length === 0) {
      // No explicit history row for this (drepId, epochNo); treat as zero voting power.
      return { value: "0" };
    }

    const row = data[0] as KoiosDrepVotingPowerRow;
    if (!row || typeof row !== "object") {
      return {
        error: {
          message: "Koios drep_voting_power_history returned non-object row",
        },
      };
    }
    ...
    clearTimeout(timeout);
    const message =
      err instanceof Error ? err.message : "Unexpected error calling Koios drep_voting_power_history";
    return {
      error: {
        message,
      },
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const drepIdFilter = parseStringParam(req.query.drep_id) ?? parseStringParam(req.query._drep_id);
  ...
  try {
    // 1) Find DRep votes that are missing votingPowerAda but have an epoch number.
    const where: NonNullable<
      Parameters<(typeof prisma.drepVote)["findMany"]>[0]
    >["where"] = {
      votingPowerAda: null,
      epochNo: { not: null },
    };
    ...
    const candidateVotes = await prisma.drepVote.findMany({
      where,
      select: {
        id: true,
        drepId: true,
      ...
      orderBy: { id: "asc" },
      take: limit * 2, // allow some duplication across (drepId, epochNo) pairs
    });
    ...
    // 2) Build distinct (drepId, epochNo) pairs, capped by `limit`.
    const pairMap = new Map<string, { drepId: string; epochNo: number; count: number }>();
    for (const v of candidateVotes) {
      if (!v.epochNo || !v.drepId) continue;
      const key = `${v.drepId}#${v.epochNo}`;
      const existing = pairMap.get(key);
      if (existing) {
        existing.count += 1;
      } else if (pairMap.size < limit) {
        pairMap.set(key, { drepId: v.drepId, epochNo: v.epochNo, count: 1 });
      }
    }
    ...
    // 3) For each pair, fetch Koios voting power and update matching DrepVote rows.
    ...
    for (const pair of pairs) {
      const { drepId, epochNo } = pair;
      const result = await fetchDrepVotingPower(drepId, epochNo);
      processedPairs += 1;
      ...
      const updateResult = await prisma.drepVote.updateMany({
        where: {
          drepId,
          epochNo,
          votingPowerAda: null,
