...
function applyFilters(
  ...
  hasScriptFilter: boolean | undefined,
  ...
  if (registeredFilter !== undefined) {
    result = result.filter((d) => Boolean(d.registered) === registeredFilter);
  }
  if (activeFilter !== undefined) {
    result = result.filter((d) => Boolean(d.active) === activeFilter);
  }
  if (hasScriptFilter !== undefined) {
    result = result.filter((d) => Boolean(d.has_script) === hasScriptFilter);
  }
...
async function handleGet(req: NextApiRequest, res: NextApiResponse) {
  const { limit, offset, registeredFilter, activeFilter, hasScriptFilter, minAmount, maxAmount } =
    getQueryParams(req);
  ...
  try {
    ...
    dreps = applyFilters(dreps, registeredFilter, activeFilter, hasScriptFilter, minAmount, maxAmount);
