...
async function verifiedBytes(
  ...
  if (Number(response.headers.get("content-length")) > limit)
    throw new Error("Anchor exceeds 1 MiB.");
  const reader = response.body?.getReader();
  if (!reader) throw new Error("Anchor has no body.");
  ...
  try {
    for (;;) {
      ...
      if (done) break;
      length += value.length;
      if (length > limit) throw new Error("Anchor exceeds 1 MiB.");
      chunks.push(value);
    }
  } finally {
    await reader.cancel();
...
export async function fetchAnchorJson(
  ...
  if (anchor.uri.startsWith("https://")) {
    bytes = await verifiedBytes(anchor.uri, anchor.hash, signal);
  } else if (anchor.uri.startsWith("ipfs://")) {
    for (const gateway of IPFS_GATEWAYS) {
      try {
        bytes = await verifiedBytes(
          gateway + anchor.uri.slice(7),
          anchor.hash,
          signal,
        );
        break;
...
export function applyPresentation(
  ...
  if (!value || typeof value !== "object")
    throw new Error("Presentation is not an object.");
  ...
  const presentations = Array.isArray(document.questions)
    ? document.questions
    : [];
  const questions = definition.questions.map((question, index): Question => {
    ...
    const prompt =
      question.prompt.trim() === "" && typeof item.prompt === "string"
        ? item.prompt
        : question.prompt;
  ...
  return {
    ...definition,
    title:
      definition.title.trim() === "" && typeof document.title === "string"
        ? document.title
        : definition.title,
    description:
...
/** Custom method semantics are deliberately unsupported until a validator is supplied. */
export function schemaAcceptsText(_value: unknown): boolean {
  void _value;
  return false;
}
